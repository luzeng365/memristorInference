...
trainSetSizeJittered = trainSetSize*9
...
dataset_jittered = np.zeros((testSetSize+trainSetSize*9,vecSize*temporalWindowSize))
labels_jittered = np.zeros((testSetSize+trainSetSize*9,1))
err = np.zeros((testSetSize))
dc3 = np.zeros((testSetSize))
vidStartIndices = []
...
dataset_jittered[0:testSetSize] = dataset_temporal[0:testSetSize]
labels_jittered[0:testSetSize] = labels_shuf[0:testSetSize]
...
for i in np.arange(trainSetSize):
    imgs = dataset_temporal[testSetSize+i]
    label = labels_shuf[testSetSize+i]
    for j in np.arange(temporalWindowSize):
        ...
        jitteredVec0[j*vecSize:(j+1)*vecSize] = img0.flatten()
        jitteredVec1[j*vecSize:(j+1)*vecSize] = img1.flatten()
        jitteredVec2[j*vecSize:(j+1)*vecSize] = img2.flatten()
        jitteredVec3[j*vecSize:(j+1)*vecSize] = img3.flatten()
        jitteredVec4[j*vecSize:(j+1)*vecSize] = img4.flatten()
        jitteredVec5[j*vecSize:(j+1)*vecSize] = img5.flatten()
        jitteredVec6[j*vecSize:(j+1)*vecSize] = img6.flatten()
        jitteredVec7[j*vecSize:(j+1)*vecSize] = img7.flatten()
        jitteredVec8[j*vecSize:(j+1)*vecSize] = img8.flatten()

    labels_jittered[testSetSize+i*9:testSetSize+(i+1)*9] = label
    dataset_jittered[testSetSize+i*9] = jitteredVec0       
    dataset_jittered[testSetSize+i*9+1] = jitteredVec1       
    dataset_jittered[testSetSize+i*9+2] = jitteredVec2    
    dataset_jittered[testSetSize+i*9+3] = jitteredVec3       
    dataset_jittered[testSetSize+i*9+4] = jitteredVec4       
    dataset_jittered[testSetSize+i*9+5] = jitteredVec5       
    dataset_jittered[testSetSize+i*9+6] = jitteredVec6       
    dataset_jittered[testSetSize+i*9+7] = jitteredVec7       
    dataset_jittered[testSetSize+i*9+8] = jitteredVec8      

# Inference with x% of the dataset as testing, and the rest as stored patterns, using the Bayesian Module (BM)
def infer_MSR(i):
    if(metric == 'dp'):
        error, dontcare1, dontcare2, dontcare3 = cdi.BM_dp(dataset_jittered[testSetSize:testSetSize+trainSetSizeJittered],dataset_jittered[i],[labels_jittered[i]],labels_jittered[testSetSize:testSetSize+trainSetSizeJittered])
    elif(metric == 'ham'):
        error, dontcare1, dontcare2, dontcare3 = cdi.BM_ham(dataset_jittered[testSetSize:testSetSize+trainSetSizeJittered],dataset_jittered[i],[labels_jittered[i]],labels_jittered[testSetSize:testSetSize+trainSetSizeJittered])
    return error
...
fid = open('MSR_results_shufflebyvideo_jittered.txt','a')
fid.write('Size: ')
