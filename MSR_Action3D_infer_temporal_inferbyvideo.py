...
def infer_MSR(i):
    if(metric == 'dp'):
        notcorrect, dontcare1, dontcare2, error = cdi.BM_dp(dataset_temporal[testSetSize:testSetSize+trainSetSize],dataset_temporal[i],[labels_shuf[i]],labels_shuf[testSetSize:testSetSize+trainSetSize])
    elif(metric == 'ham'):
        notcorrect, dontcare1, dontcare2, error = cdi.BM_ham(dataset_temporal[testSetSize:testSetSize+trainSetSize],dataset_temporal[i],[labels_shuf[i]],labels_shuf[testSetSize:testSetSize+trainSetSize])
...
for i in np.arange(testSetSize):
    if ((inferredFrames + vidLengths[i]) >= testSetSize):
        break
    notcorrr = notcorr[inferredFrames:inferredFrames + vidLengths[i]]
    errr = err[inferredFrames:inferredFrames + vidLengths[i]]
    if(metric == 'dp'):
        idx = np.argmax(errr) 
        ...
        idx = np.argmin(errr) 
        actualerror[i] = notcorrr[idx]
    inferredFrames = inferredFrames + vidLengths[i]

accuracy = 1-sum(actualerror)/float(i)
print(1-sum(notcorr)/float(testSetSize))
# Print accuracy over the whole test set
print('Accuracy_' + str(sizeY) + 'x' + str(sizeX) + ': ')
print(accuracy)

# Save results and parameters to file
fid = open('MSR_results_nojitter_splits_inferbyvideo.txt','a')
fid.write('Size: ')
